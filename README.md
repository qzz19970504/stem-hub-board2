...
基于 STM32F103C8、STM32 HAL 和 FreeRTOS 的板2固件。UART1 在 AT 命令模式和透明传输模式之间切换，UART2、UART3 可单独或同时作为透明目标；应用层同时提供三路高有效 NMOS 输出、一路 25 kHz PWM 和运行状态 LED。
...
- 通过 `AT+TRANS=1|2|1&2` 进入 UART2、UART3 单目标或双目标透明传输。
- 透明模式下 UART1 字节不经过 CRLF 拼帧或 AT 解析，包括 `0x00` 和形似 AT 命令的数据。
- 前后至少静默 1 ms 的 `+++` 退出透明模式，失败的候选字节会原样补发。
...
三路 UART 均为 `9600、8N1、无硬件流控`。

| 功能 | MCU 资源 | 有效电平或方向 | 软件启动后的默认状态 |
| --- | --- | --- | --- |
| UART1 | PA9=TX、PA10=RX | 主控制串口 | 接收已启动 |
| UART2 | PA2=TX、PA3=RX | 透明目标通道 1 | 未选择，接收已启动 |
| UART3 | PB10=TX、PB11=RX | 透明目标通道 2 | 未选择，接收已启动 |
...
UART1 数据先进入 64 B 中断接收块，再由 `HAL_UARTEx_RxEventCallback()` 搬入 UART1 的 256 B 环形缓冲。回调同时记录每个接收块前后是否存在 UART IDLE 边界，释放 UART1 信号量并立即重新挂接接收。9600 8N1 的一个字符时间约为 1.04 ms，因此 IDLE 边界可保守证明至少 1 ms 静默。

`atTask` 被唤醒后按互斥状态处理：

- AT 模式逐字节拼接 CRLF 帧并执行严格 AT 解析；命令必须全大写、无空白并以 `\r\n` 结束。
- `AT+TRANS=1`、`2`、`1&2` 返回 `OK\r\n` 后切换到对应透明目标。
- 透明模式完全绕过 CRLF 拼帧和 AT Parser，所有普通字节原样发送到目标。
- 只有前后均有 IDLE 边界的 `+++` 被 MCU 消耗并退出透明模式；退出后返回 `OK\r\n`。
- 不满足保护时间的 `+++`、`abc+++def` 和 `++++` 都作为普通数据完整转发。
...
退出透明模式时会同时清除目标并清空 UART2、UART3 的待处理 RX 数据。bridge 状态互斥锁保证“检查目标、读取缓冲、发送事件”和“退出并清缓冲”不会交错，因此退出确认后不会继续回放旧数据。
...
flowchart LR
    ...
    R1 --> Mode["atTask<br/>AT / 透明模式路由"]
    Mode --> AT["CRLF + AT 解析"]
    Mode --> Transparent["透明状态机<br/>+++ guard"]
    R2 --> Bridge["bridgeTask<br/>最多 32 B HEX 编码"]
    R3 --> Bridge

    AT --> Output["app_output<br/>NMOS / PWM"]
    Transparent -->|"原始字节"| TX["UART TX Mutex"]
    AT -->|"OK / ERROR"| TX
...
ISR 只做有限工作：复制本次接收数据、记录 UART1 IDLE 分段和环形缓冲溢出、释放信号量、重新挂接 Receive-to-Idle。解析、透明模式判定、HEX 编码和阻塞式发送均在任务上下文执行。
...
| `atTask` | AboveNormal | 1024 B | UART1 空闲分段、AT/透明模式路由、命令执行和下行透传 |
...
- bridge 事件标志：记录透明模式当前选择的 UART2、UART3 目标。
- UART TX 互斥锁：串行化三路 UART 的任务态发送。
- bridge 状态互斥锁：串行化目标切换、清队列和回传事务。
...
| `app_transparent_mode` | 互斥模式与无损 `+++` 保护时间状态机 | 纯 C，可主机测试 |
| `app_uart_chunk_queue` | 保存 UART1 Receive-to-Idle 分段及前后静默证据 | 纯 C，单生产者/单消费者 |
...
| `AT+UARTTX=<HEX>` | 兼容保留；AT 模式没有活动目标时返回 `+ERROR:UART_DISABLED` |
...
进入透明模式后，所有 AT 字符串都失去命令含义并作为 payload 转发。发送前后各静默至少 1 ms 的 `+++` 可退出，MCU 随后返回 `OK\r\n`。旧的 `AT+UART2/UART3/UART2&3=ON/OFF` 六条命令已移除，在 AT 模式下返回 `+ERROR:PARSE\r\n`。
...
- AT 模式才使用 CRLF 拼帧；透明模式按 Receive-to-Idle 块原样转发，CRLF 和 NUL 均无特殊含义。
- 成功退出透明模式会清空 UART2、UART3 的待回传数据和目标位。
