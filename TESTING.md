...
在 PB9/TIM4_CH4 接示波器或逻辑分析仪。TIM4 时钟为 64 MHz，PSC=0、ARR=2559，因此频率为 25 kHz。
...
允许测量误差应根据仪器采样率和板级时钟误差确定。功能验收重点是频率接近 25 kHz，且四个合法占空比与命令一致。

- [ ] 测试结束后发送 `AT+PWM=0\r\n`。

### 渐变、持久化和呼吸测试

1. 设置 `AT+PWM_TIME=500`，从 PWM 0 设置到 60，确认约 500 ms 平滑上升；60→0 同样平滑下降。
2. 渐变途中发送新目标，确认从当前亮度转向新目标，AT 串口仍能立即响应。
3. 设置 `AT+PWM_TIME=800` 后复位，查询状态确认 `PWM_TIME=800`。
4. 开启 18V 后发送 `AT+BREATH_TEST=ON`，确认亮度在 0%～100% 间循环。
5. 呼吸期间 `AT+PWM=30` 应返回 `+ERROR:BREATH_ACTIVE`；`AT+18V=OFF` 应立即熄灯并停止呼吸。
...
该测试需要制造任务来不及消费的情况，例如在调试器中暂停消费者任务后向对应 UART 高速注入超过 256 B，再恢复运行。普通 9600 短帧不一定稳定触发。
